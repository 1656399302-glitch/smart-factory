        ...
        >
          ...
          <el-table-column label="名称" prop="name" width="150">
            <template slot-scope="scope">
              {{
                scope.row.category
                  ? `${scope.row.name}（${scope.row.category}）`
                  : scope.row.name
              }}
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <dict-tag
                ...
                :value="scope.row[usageStatusName]"
              />
            </template>
          </el-table-column>
        </el-table>
        ...
        .then(() => {
          this.updateSub({
            ...
            if (this.mainList[parentIndex][this.mainListName].length === 0) {
              ...
              this.mainList[parentIndex][this.usageStatusName] = 0;
            }
          });
        });
    },
        ...
        if (valid) {
          if (this.form[this.mainId] != null) {
            this.updateSub({
              ...
              [this.subMainId]: this.form[this.mainId],
            }).then((response) => {
              ...
              // 如果使用情况为未使用，则设为已使用
              if (item[this.usageStatusName] == 0) {
                // this.$set触发视图更新
                this.$set(item, this.usageStatusName, 1);
              }
              this.open = false;
              // this.getList();
              this.subLoading = true;
