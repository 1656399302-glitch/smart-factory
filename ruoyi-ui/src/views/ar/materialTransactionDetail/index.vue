...
<script>
import {
  listMaterialTransactionDetail,
  getMaterialTransactionDetail,
  delMaterialTransactionDetail,
  addMaterialTransactionDetail,
  updateMaterialTransactionDetail,
} from "@/api/ar/materialTransactionDetail";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm(
          '是否确认删除材料清单明细编号为"' + materialIds + '"的数据项？'
        )
        .then(function () {
    ...
    handleExport() {
      this.download(
        "ar/materialTransactionDetail/export",
        {
          ...this.queryParams,
        },
